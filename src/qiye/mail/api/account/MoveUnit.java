package qiye.mail.api.account;

import java.io.UnsupportedEncodingException;

import rsa.HttpPost;
import rsa.RSASignatureToQiye;

public class MoveUnit {

	public static void main(String[] args) throws UnsupportedEncodingException {
		//根据企业实际修改，priKey为私钥，domain为域名，product即cid由网易提供
		String priKey = "";
		String domain = "abc.com";
		String product = "abc_com";
		
		String account_name = "zhangsan";
		String unit_id = "default";
		String time = System.currentTimeMillis() + "";
		
		String url = "https://apihz.qiye.163.com/qiyeservice/api/account/moveUnit";
		String sign = "account_name=" + account_name + "&domain=" + domain + "&product=" + product + "&time=" + time + "&unit_id=" + unit_id;
		System.out.println(sign);
		sign = RSASignatureToQiye.generateSigature(priKey, sign);
		System.out.println(sign);
		url = url + "?" + "account_name=" + account_name + "&domain="
				+ domain  + "&product=" + product + "&sign=" + sign + "&time=" + time + "&unit_id=" + unit_id;
		System.out.println(url);
		HttpPost hp = new HttpPost();
		String res = hp.post(url);
		System.out.print(res);
		
	}

}
